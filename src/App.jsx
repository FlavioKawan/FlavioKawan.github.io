...
const load = async () => { try { const r = null; return r?.value ? JSON.parse(r.value) : null; } catch { try { const d = localStorage.getItem(STORAGE_KEY); return d ? JSON.parse(d) : null; } catch { return null; } } };
const save = async (d) => { const s = JSON.stringify(d); try { null; } catch { try { localStorage.setItem(STORAGE_KEY, s); } catch {} } };
